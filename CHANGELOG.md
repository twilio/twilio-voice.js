...
- Fixed an [issue](https://github.com/twilio/twilio-voice.js/issues/272) where the local DTMF tones played back to the caller by `call.sendDigits()` fell out of sync with the tones sent to the recipient around a pause character (`w`). Local playback advanced past a pause after only the 200ms tone gap instead of the 500ms pause used on the wire, so depending on how many digits preceded the pause the caller could hear the tones after it either earlier or later than the recipient received them, and the error accumulated with each `w`. This is common in PIN and IVR entry such as `sendDigits('1w1234')`. Local playback now advances past a pause on the same schedule as the DTMF sent over the wire. Thanks @a-lindsay for reporting this.
