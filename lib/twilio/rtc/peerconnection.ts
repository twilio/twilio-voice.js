...
class PeerConnection extends EventEmitter {
  ...
  _audioContext: any;
  _audioHelper: any;
  _audioProcessorEventObserver: any;
  _hasIceCandidates: boolean;
  _hasIceGatheringFailures: boolean;
  _iceGatheringTimeoutId: any;
  _masterAudio: any;
  _masterAudioDeviceId: any;
  _mediaStreamSource: any;
  _dtmfSender: any;
  _dtmfSenderUnsupported: boolean;
  _callEvents: any[];
  _nextTimeToPublish: number;
  _onAnswerOrRinging: Function;
  _onHangup: Function;
  _remoteStream: any;
  _shouldManageStream: boolean;
  _iceState: string;
  _onAudioProcessorAdded: (isRemote: boolean) => void;
  _onAudioProcessorRemoved: (isRemote: boolean) => void;
  _sender: any;
  _answerSdp: string;
  _inputAnalyser: any;
  _outputAnalyser: any;
  _inputAnalyser2: any;
  _outputAnalyser2: any;
  _inputStreamSource: any;
  _outputStreamSource: any;
  _uri: string;

  // Static properties
  static protocol: any = ((() => RTCPC.test() ? new (RTCPC as any)() : null))();
  static enabled: boolean = RTCPC.test();

  constructor(audioHelper: any, pstream: any, options?: any) {
    super();
    ...
    this._isSinkSupported = !!AudioContext &&
      typeof HTMLAudioElement !== 'undefined' && (HTMLAudioElement.prototype as any).setSinkId;
    ...
    this._onAudioProcessorAdded = (isRemote: boolean) => {
      this._handleAudioProcessorEvent(isRemote, true);
    };
    this._onAudioProcessorRemoved = (isRemote: boolean) => {
   ...
   */
  openDefaultDeviceWithConstraints(constraints: any): Promise<any> {
   ...
   */
  setInputTracksFromStream(stream: any): Promise<any> {
    const self = this;
  ...
  _createAnalyser(audioContext: any, options?: any): any {
  ...
  _setVolumeHandler(handler: Function): void {
    this.removeAllListeners('volume');
    this.on('volume', handler as any);
  }

  _startPollingVolume(): void {
    ...
    setTimeout(function emitVolume() {
      ...
      self.emit('volume', inputVolume / 255, outputVolume / 255, inputVolume2, outputVolume2);
   ...
   * @private
   */
  _updateInputStreamSource(stream: any): void {
   ...
   * @private
   */
  _updateOutputStreamSource(stream: any): void {
 ...
 * @private
 */
  _setInputTracksFromStream(shouldClone: boolean, newStream: any): Promise<any> {
  if (!newStream) {
  ...
  _onInputDevicesChanged(): void {
  if (!this.stream) { return; }

  // If all of our active tracks are ended, then our active input was lost
  const activeInputWasLost = this.stream.getAudioTracks().every((track: any) => track.readyState === 'ended');
  ...
  _onIceGatheringFailure(type: string): void {
  this._hasIceGatheringFailures = true;
  this.emit('icegatheringfailure', type);
  }

  _onMediaConnectionStateChange(newState: string): void {
  ...
  _setSinkIds(sinkIds: any): Promise<any> {
   ...
   */
  _startIceGatheringTimeout(): void {
  this._stopIceGatheringTimeout();
   ...
   */
  _stopIceGatheringTimeout(): void {
  clearInterval(this._iceGatheringTimeoutId);
  }

  _updateAudioOutputs(): Promise<any> {
  const addedOutputIds = Array.from(this.sinkIds).filter((id: string) => {
    return !this.outputs.has(id);
  });

  const removedOutputIds = Array.from(this.outputs.keys()).filter((id: string) => {
  ...
  _createAudio(arr?: any): any {
  const audio = new Audio(arr);
  this.emit('audio', audio);
  return audio;
  }

  _createAudioOutput(id: any): Promise<any> {
  let dest = null;
  ...
  _disableOutput(pc: any, id: any): void {
   ...
   */
  _reassignMasterOutput(pc: any, masterId: any): Promise<any> {
  ...
  _removeAudioOutput(id: any): Promise<any> {
   ...
   */
  _onAddTrack(pc: any, stream: any): void {
   ...
   */
  _fallbackOnAddTrack(pc: any, stream: any): void {
  ...
  _setEncodingParameters(enableDscp: any): void {
  if (!enableDscp
      || !this._sender
  ...
  _setupPeerConnection(rtcConfiguration: any): any {
  const self = this;
  ...
  const supportedCodecs = (RTCRtpReceiver as any).getCapabilities('audio').codecs;
  ...
  version.pc[eventName] = (event: any) => {
  ...
  _maybeSetIceAggressiveNomination(sdp: any): any {
  ...
  this.version.pc.onsignalingstatechange = () => {
    ...
    this.emit('signalingstatechange', pc.signalingState);
  };
  ...
  pc.onconnectionstatechange = (event: any) => {
    ...
    this.emit('pcconnectionstatechange', state);
  ...
  pc.oniceconnectionstatechange = () => {
    ...
    this.emit('iceconnectionstatechange', pc.iceConnectionState);
    this._onMediaConnectionStateChange(pc.iceConnectionState);
  };
  }

  _initializeMediaStream(rtcConfiguration: any): boolean {
   ...
   * @private
   */
  _removeReconnectionListeners(): void {
  if (this.pstream) {
   ...
   * @private
   */
  _setupRTCDtlsTransportListener(): void {
  ...
  const handler = () => {
    ...
    this.emit('dtlstransportstatechange', dtlsTransport.state);
  };
   ...
   * @private
   */
  _setupRTCIceTransportListener(): void {
  ...
  iceTransport.onselectedcandidatepairchange = () =>
    this.emit('selectedcandidatepairchange', iceTransport.getSelectedCandidatePair());
  }
  ...
  this.version.createOffer(this.options.maxAverageBitrate, { iceRestart: true }).then(() => {
    ...
    this._onAnswerOrRinging = (payload: any) => {
      this._removeReconnectionListeners();
      ...
      if (this.status !== 'closed') {
        this.version.processAnswer(this.codecPreferences, sdp, null, (err: any) => {
  ...
  makeOutgoingCall(params: any, signalingReconnectToken: any, callsid: any, rtcConfiguration: any, onMediaStarted: any): void {
  ...
  answerIncomingCall(callSid: any, sdp: any, rtcConfiguration: any, onMediaStarted: any): void {
  if (!this._initializeMediaStream(rtcConfiguration)) {
    return;
  }
  ...
  reject(callSid: any): void {
  this.callSid = callSid;
  }

  ignore(callSid: any): void {
  this.callSid = callSid;
  }
  ...
  if (typeof pc.getSenders === 'function' && (typeof RTCDTMFSender === 'function' || typeof (globalThis as any).RTCDtmfSender === 'function')) {
    const chosenSender = pc.getSenders().find((sender: any) => sender.dtmf);
    if (chosenSender) {
  ...
  if (typeof pc.createDTMFSender === 'function' && typeof pc.getLocalStreams === 'function') {
    const track = pc.getLocalStreams().map((stream: any) => {
  ...
  _canStopMediaStreamTrack(): boolean {
    return typeof MediaStreamTrack.prototype.stop === 'function';
  }

  _getAudioTracks(stream: any): any {
   ...
   */
  _getRTCIceTransport(): any {
  const dtlsTransport = this.getRTCDtlsTransport();
  return dtlsTransport && dtlsTransport.iceTransport || null;
  }

  _handleAudioProcessorEvent(isRemote: any, isAddProcessor: any): void {
...
function addStream(pc: any, stream: any): void {
...
function cloneStream(oldStream: any, _MediaStream: any): any {
  let newStream;
  if (_MediaStream) {
    ...
    newStream = new (globalThis as any).webkitMediaStream();
  }
...
function removeStream(pc: any, stream: any): void {
  if (typeof pc.removeTrack === 'function') {
    pc.getSenders().forEach((sender: any) => { pc.removeTrack(sender); });
  } else {
 ...
 */
function setAudioSource(audio: any, stream: any, audioHelper: any): Promise<any> {
  return audioHelper._maybeCreateRemoteProcessedStream(stream).then((maybeProcessedStream: any) => {
