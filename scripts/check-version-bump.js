 ...
 * @param {string} version - an exact version, e.g. "2.18.4" or "2.18.4-rc1"
...
function parse(version) {
  ...
  return {
    ...
    // Splitting on "." handles both a dotted "rc.1" and the dotless "rc2" this
    // repo's tags actually use (e.g. 2.18.2-rc2).
...
function comparePrerelease(a, b) {
  ...
  // An absent prerelease outranks any prerelease: 2.18.0 > 2.18.0-rc1. This is
...
function compare(a, b) {
  ...
  for (const [version, parsed] of [[a, left], [b, right]]) {
    if (!parsed) {
      throw new Error(`${version} is not an exact version (expected 2.18.4 or 2.18.4-rc1).`);
    }
  }
